...
if uploaded_file:
    try:
        ...
        if not required_cols.issubset(df.columns):
            ...
            # Фильтр по статусу (не выводим в таблице)
            ...
            # --- Вычисляем площадь для расчётов ---
            ...
            if not df_filtered.empty:
                ...
                # Только нужные колонки (без "Статус" и "Вид помещения")
                ...
                preview_df = preview_df.rename(columns={"Площадь для расчёта": "Площадь общая"})
                ...
                if df_filtered.empty:
                    ...
                    with zipfile.ZipFile(buffer, "w") as zf:
                        ...
                        for dept in df_filtered["Подразделение"].unique():
                            ...
                            # Переименовываем колонку для отображения
                            df_dept = df_dept.rename(columns={"Площадь для расчёта": "Площадь общая"})
